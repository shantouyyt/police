...
public class OutPoliceDal {
	...
	public jqOutInfo<OutPoliceInfo> ListDistinct(OutPoliceInfo info, int iDisplayStart,
			int iDisplayLength) {

		String sql = "select DISTINCT InPoliceID,CreateDate from OutPolice  where 1=1 ";
		...
		String countsql = "select count(DISTINCT InPoliceID,CreateDate) from OutPolice  where 1=1 ";
		int count = JDBCJNDI.count(countsql + sb.toString());
